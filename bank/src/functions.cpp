...
// message function use to show what user can do
void message()
{
    cout << "<<<<<<<<<<<>>>>>>>>>>" << endl;
...
// checking username to add new customer
bool check_username(string name, vector<customer> &v_customer)
{
    try
    {
        ...
        // use alphas ascii code for checking that name is English or not
            ...
            if (request[j] == '\0')
            {
                ...
                if (status == WITHDRAW)
                {
                    if (v_customer[i].transaction(-money, WITHDRAW, v_customer,AT))
                    {
                        ...
                        temp.set_source(v_customer[i].get_username()); //store source account name

                        AT.push_back(temp); //store withdraw transactions in bank list
                    }
                }
                else if (status == DEPOSIT)
                {
                    if (v_customer[i].transaction(money, DEPOSIT, v_customer,AT))
                    {
                        banktransaction temp(money);                   // make new transaction
                        temp.set_source(v_customer[i].get_username()); //store source account name
                        AT.push_back(temp);                            //store deposit transactions in bank list
                    }
                }
                v_customer[i].save_transactions(AT); //store number of vector<banktransaction> in transaction list customer
                break;
            }
            j++;
        }
...
void transfer(string request, vector<customer> &v_customer, vector<banktransaction> &AT)
{
    ...
    try
    {
        ...
        temp.set_destination(v_customer[j].get_username()); //store destination account name
        AT.push_back(temp);                                 //store deposit transactions in bank list
        v_customer[j].save_transactions(AT);                //store number of vector<banktransaction> in transaction list customer
...
bool show(string request, vector<customer> &v_customer, vector<banktransaction> &AT)
{
    ...
    try
    {
        ...
        case 0: // username
        {
            for (auto item : v_customer)
            {
                if (item.get_username() == request)
                {
                    ...
                    cout << "------------------------" << endl;
                    cout << "Balance: " << item.get_stock() << " Rials" << endl;
                    cout << "------------------------" << endl;
                    cout << "The amount of debt to the bank: " << -(item.get_borrow()) << " Rials" << endl;
                    cout << "------------------------" << endl;
        ...
        case 3://bank
        {
            ...
            cout << "current balance: " << current_balance << " Rials" << endl;
            cout << "pure balance: " << pure_balance << " Rials" << endl;
            cout << "loan amount: " << -loan << " Rials" << endl;
            return true;
        }
        default:
            break;
